    ...
    for _ in range(args.num_epoch):
        for imgs, features in train_loader:
            ...
            if args.model == 'cgan':
                ...
                r = torch.cat([data_dic['labels'], torch.ones((args.batch_size, 1))], dim=1)
                f = torch.cat([1. - data_dic['fake_labels'], torch.zeros((args.batch_size, 1))], dim=1)
                ...
                df = discriminator(g, data_dic['fake_labels'])
                f = torch.cat([data_dic['fake_labels'], torch.ones((args.batch_size, 1))], dim=1)
